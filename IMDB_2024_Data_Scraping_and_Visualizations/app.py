...
if file:
    ...
    def visualizer():
        ...
        with tab1:
            ...
            genre_options = sorted(set(genre for sublist in df['Genre'].dropna().str.split(', ') for genre in sublist))
        ...
        with tab2:
            ...
            #Explode genre for plots
            df_exploded_vis = df.copy()
            df_exploded_vis['Genre'] = df_exploded_vis['Genre'].str.split(', ')
            df_exploded_vis = df_exploded_vis.explode('Genre')

            #Genre Distribution
            genre_counts = df_exploded_vis['Genre'].value_counts()
            ...
            df_exploded_vis['Runtime'] = df_exploded_vis['Runtime'].astype(int)
            avg_duration = df_exploded_vis.groupby('Genre')['Runtime'].mean().sort_values(ascending=False).reset_index()
            avg_duration.columns = ['Genre', 'Average Duration']
            fig = px.bar(
                avg_duration,
                x='Average Duration',
                y='Genre',
            ...
            df_exploded_vis['Vote_Counts'] = df_exploded_vis['Vote_Counts'].astype(float)
            average_votes = df_exploded_vis.groupby('Genre', as_index=False)['Vote_Counts'].mean()
            ...
            #Most Popular Genres
            genre_votes = df_exploded_vis.groupby('Genre')['Vote_Counts'].sum().reset_index()
            fig = px.pie(
                genre_votes,
                names='Genre',
                values='Vote_Counts',
            ...
            top_rated = df_exploded_vis.sort_values(by = "Ratings", ascending=False).drop_duplicates('Genre')
            ...
            average_ratings = df_exploded_vis.groupby('Genre')['Ratings'].mean().reset_index()
